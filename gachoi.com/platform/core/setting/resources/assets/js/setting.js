...
class SettingManagement {
    init() {
        ...
        $('.delete-generate-thumbnails-trigger-button').on('click', event => {
            event.preventDefault();
            let _self = $(event.currentTarget);
            let defaultText = _self.text();
            ...
            $('#delete-generate-thumbnails-modal').modal('show');
            _self.text(defaultText);
        });
        ...
        $('#delete-generate-thumbnails-button').on('click', event => {
            event.preventDefault();
            let _self = $(event.currentTarget);
            ...
            $.ajax({
                type: 'POST',
                url: route('settings.media.delete-generate-thumbnails'),
                success: res => {
                    if (!res.error) {
                        Botble.showSuccess(res.message);
                    } else {
