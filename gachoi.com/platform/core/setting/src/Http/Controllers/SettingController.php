...
class SettingController extends BaseController
{
     ...
     * @return BaseHttpResponse
     */
    public function deleteGenerateThumbnails(MediaFileInterface $fileRepository, BaseHttpResponse $response)
    {
        @ini_set('max_execution_time', -1);
        @ini_set('memory_limit', -1);
        ...
        if ($errors) {
            return $response
                ->setError()
                ->setMessage(trans('core/setting::setting.delete_generate_thumbnails_error', ['count' => count($errors)]));
        }

        return $response->setMessage(trans('core/setting::setting.delete_generate_thumbnails_success', ['count' => count($files)]));
    }
}
